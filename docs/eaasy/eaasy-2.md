...
项目部署的整体流程是，复制和解压压缩包（包含了Ansible环境、K8S环境，项目镜像，项目依赖中间件环境），安装基础环境。通过Ansilbe脚本，将基础应用和项目应用部署到K8S环境中。
...
此方案废弃，因为获取内容要进过一层转化服务，AOP的方式同时增大了系统的性能开销。所以数据库通过在脚本仓库中添加国际化文件，在Ansilbe脚本中添加国际化参数的方式，控制数据的初始化加载。
...
通过把逻辑从脚本中的判断，放在通过项目结构的方式解决，并且即使忘记在Ansilbe中添加参数，也能加载原始的文件，方案更加的优雅。这个方案缺点就是不需要国际化的文件也要创建一份，增加了一点代码量，但是带来的好处是值得的。
